...
export default function TaxonomyIndex({ categories = [], disciplines = [], categoryTypes = [] }) {
    ...
    const filteredCategories = useMemo(() => {
        ...
        return categories.filter((item) => {
            const typeMatch = categoryTypeFilter === 'all' ? true : item.type === categoryTypeFilter;
            const statusMatch = categoryStatusFilter === 'all'
                ? true
            ...
            const searchMatch = search === ''
                ...
                : String(item.label).toLowerCase().includes(search)
                    ...
                    || String(categoryTypeLabel[item.type] || item.type).toLowerCase().includes(search);
    ...
    const filteredDisciplines = useMemo(() => {
        ...
        return disciplines.filter((item) => {
            const statusMatch = disciplineStatusFilter === 'all'
                ? true
    ...
    const categoryColumns = useMemo(() => [
        {
            ...
            dataIndex: 'type',
            key: 'type',
            render: (value) => highlightMatch(categoryTypeLabel[value] || value, categorySearch),
        },
        {
            title: 'Stored Value',
            dataIndex: 'value',
    ...
    const disciplineColumns = useMemo(() => [
        {
            ...
            key: 'code',
            width: 90,
            render: (value) => <Tag color="purple">{highlightMatch(value, disciplineSearch)}</Tag>,
        },
        {
            title: 'Discipline',
            dataIndex: 'name',
    ...
    function openCategoryCreate() {
        ...
        categoryForm.setData({ type: 'data_type', value: '', label: '', sort_order: 0, is_active: true });
    ...
    function submitCategory() {
        const payload = {
            ...categoryForm.data,
            value: String(categoryForm.data.value || '').trim().toLowerCase().replace(/\s+/g, '_'),
        ...
        if (editingCategory) {
            router.put(route('admin.taxonomy.categories.update', editingCategory.id), payload, {
                preserveScroll: true,
                onSuccess: () => { setCategoryProcessing(false); closeCategoryEditor(); },
                onError: handleError,
            });
            return;
        }
    ...
    function submitDiscipline() {
        ...
        const duplicate = disciplines.find((item) => (
            String(item.code) === payload.code
            && item.id !== editingDiscipline?.id
        ));
        ...
        if (editingDiscipline) {
            router.put(route('admin.taxonomy.disciplines.update', editingDiscipline.id), payload, {
                preserveScroll: true,
                onSuccess: () => { setDisciplineProcessing(false); closeDisciplineEditor(); },
                onError: handleError,
            });
            return;
        }
        ...
        <AuthenticatedLayout
            ...
            header={(
                <AdminPageHeader
                    ...
                    actions={(
                        <Space>
                            <Button size="large" icon={<PlusOutlined />} onClick={openCategoryCreate}>Add Category</Button>
                            <Button size="large" type="primary" icon={<PlusOutlined />} onClick={openDisciplineCreate}>Add Discipline</Button>
                        </Space>
                    )}
                />
            )}
        >
            ...
            >
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                    ...
                    />
                    {categoryForm.errors.label && <Typography.Text type="danger">{categoryForm.errors.label}</Typography.Text>}
                    <Input
                        size="large"
                    ...
                    />
                    {categoryForm.errors.value && <Typography.Text type="danger">{categoryForm.errors.value}</Typography.Text>}
                    <InputNumber
                        size="large"
                        min={0}
                        style={{ width: '100%' }}
            ...
            >
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                    <Input
                        ...
                        onChange={(event) => disciplineForm.setData('code', String(event.target.value || '').replace(/\D+/g, '').slice(0, 2))}
                        placeholder="Code (2 digits, e.g., 47)"
                        status={disciplineForm.errors.code ? 'error' : ''}
                    />
                    {disciplineForm.errors.code && <Typography.Text type="danger">{disciplineForm.errors.code}</Typography.Text>}
                    <Input
                        size="large"
                    ...
                    />
                    {disciplineForm.errors.name && <Typography.Text type="danger">{disciplineForm.errors.name}</Typography.Text>}
                    <InputNumber
                        size="large"
                        min={0}
                        style={{ width: '100%' }}
