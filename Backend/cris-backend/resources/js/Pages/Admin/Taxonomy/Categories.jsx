...
export default function CategoriesIndex({ categories = [], categoryTypes = [] }) {
        ...
        () => [
            {
                ...
                dataIndex: 'type',
                key: 'type',
                render: (v) => highlightMatch(categoryTypeLabel[v] || v, search),
            },
            {
                title: 'Stored Value',
                dataIndex: 'value',
                key: 'value',
                responsive: ['sm'],
                render: (v) => <Tag color="blue">{highlightMatch(v, search)}</Tag>,
            },
            {
                title: 'Label',
                dataIndex: 'label',
    ...
    function submit() {
        ...
        const duplicate = categories.find(
            (item) => String(item.value).toLowerCase() === payload.value && item.id !== editing?.id
        );
        if (duplicate) {
            message.error('Duplicate category value. Please use a unique stored value.');
            return;
        }
        ...
        >
                        ...
                        locale={{
                            emptyText: (
                                <EmptyState
                                    ...
                                    description="Adjust filters or create a new research category."
                                />
                            ),
                        }}
                    />
            ...
            >
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                    ...
                    />
                    {form.errors.label && (
                        <Typography.Text type="danger">{form.errors.label}</Typography.Text>
                    )}
                    <Input
                        size="large"
                    ...
                    />
                    {form.errors.value && (
                        <Typography.Text type="danger">{form.errors.value}</Typography.Text>
                    )}
                    <InputNumber
                        size="large"
                        min={0}
