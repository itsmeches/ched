...
export default function UsersIndex({ users, filters, roleCounts, institutions }) {
    ...
    const categoryItems = useMemo(
        () => [
            {
                key: 'all',
                label: roleLabelMap.all,
                count: roleCounts?.all ?? 0,
                color: accentPrimary,
            },
            {
                key: 'pending',
                label: roleLabelMap.pending,
                count: roleCounts?.pending ?? 0,
                color: '#d97706',
            },
            {
                key: 'super_admin',
                label: roleLabelMap.super_admin,
                count: roleCounts?.super_admin ?? 0,
                color: '#7c3aed',
            },
            {
                key: 'ched',
                label: roleLabelMap.ched,
                count: roleCounts?.ched ?? 0,
                color: dark ? '#60a5fa' : '#0033a0',
            },
            {
                key: 'hei',
                label: roleLabelMap.hei,
                count: roleCounts?.hei ?? 0,
                color: dark ? '#60a5fa' : '#0047d4',
            },
            {
                key: 'faculty',
                label: roleLabelMap.faculty,
                count: roleCounts?.faculty ?? 0,
                color: '#0891b2',
            },
            {
                key: 'student',
                label: roleLabelMap.student,
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                key: 'role',
                render: (value) => (
                    <Tag color={roleColorMap[value]}>{value.replace('_', ' ').toUpperCase()}</Tag>
                ),
            },
            {
                title: 'Institution',
            ...
            },
            {
                ...
                render: (_, user) => (
                    <Space>
                        {deactivated ? (
                            <Button type="link" onClick={() => restoreUser(user.id)}>
                                Restore
                            </Button>
                        ) : (
                            <>
                                ...
                                <Button danger type="link" onClick={() => deleteUser(user.id)}>
                                    Deactivate
                                </Button>
                            </>
                        )}
            ...
            <div className="space-y-4">
                ...
                >
                    {selectedUserIds.length > 0 && (
                        <div
                            className={`mb-4 rounded-2xl border px-4 py-3 ${dark ? 'border-[#1e2d47] bg-[#0d1526]' : 'border-blue-200 bg-blue-50/70'}`}
                        >
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                <Space wrap>
                                    ...
                                    <Typography.Text type="secondary">
                                        Bulk actions apply to the checked user accounts in the
                                        current table.
                                    </Typography.Text>
                                </Space>
                                <Space wrap>
                                    ...
                                    {deactivated ? (
                                        <Button onClick={() => runBulkAction('restore')}>
                                            Restore Selected
                                        </Button>
                                    ) : (
                                        <Button danger onClick={() => runBulkAction('deactivate')}>
                                            Deactivate Selected
                                        </Button>
                                    )}
                                </Space>
                        ...
                        locale={{
                            emptyText: (
                                <EmptyState
                                    ...
                                    description="Adjust role, institution, date, or search terms to find users."
                                />
                            ),
                        }}
                    />
