...
export default function SuperAdminDashboard({
    ...
    const submit = (event) => {
        ...
        post(route('admin.users.store'), {
            preserveScroll: true,
            onSuccess: () =>
                reset('name', 'email', 'password', 'password_confirmation', 'institution_id'),
        });
    };
        ...
        >
            <Space wrap className="quick-action-cluster">
                <Link href={route('admin.users.index')}>
                    <Button className="quick-action-primary" icon={<TeamOutlined />}>
                        Open User Management
                    </Button>
                </Link>
                <Link href={route('admin.institutions.index')}>
                    <Button className="quick-action-secondary" icon={<BankOutlined />}>
                        Open Institutions
                    </Button>
                </Link>
                <Link href={route('admin.keywords.index')}>
                    <Button className="quick-action-secondary" icon={<BookOutlined />}>
                        Open Keywords
                    </Button>
                </Link>
                <Link href={route('admin.taxonomy.index')}>
                    <Button className="quick-action-secondary" icon={<AppstoreOutlined />}>
                        Open Taxonomy
                    </Button>
                </Link>
            </Space>
    ...
    return (
        <AuthenticatedLayout header={<AdminPageHeader title="Super Admin Dashboard" />}>
            ...
            <div className="space-y-5">
                ...
                <div className="sm:hidden">{renderAdminShortcuts()}</div>

                <div className="hidden sm:block">{renderAdminShortcuts()}</div>
