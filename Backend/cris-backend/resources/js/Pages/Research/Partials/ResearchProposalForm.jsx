    ...
    const removeCoAuthorRow = (index) => {
        const next =
            coAuthorRows.length === 1
                ? [{ name: '', email: '', phone: '' }]
                : coAuthorRows.filter((_, i) => i !== index);
        setCoAuthorRows(next);
        const { co_authors, co_author_emails, co_author_phones } = serializeCoAuthors(next);
        setData((prev) => ({ ...prev, co_authors, co_author_emails, co_author_phones }));
    };
        ...
        <Form layout="vertical" onSubmitCapture={onSubmit} requiredMark={false}>
            ...
            <Row gutter={16}>
                ...
                <Col xs={24} md={8}>
                    <Form.Item
                        label="Author Email"
                        validateStatus={errors.author_email ? 'error' : ''}
                        help={errors.author_email}
                    >
                        <Input
                            size="large"
                            type="email"
                ...
                <Col xs={24} md={8}>
                    <Form.Item
                        label="Author Phone"
                        validateStatus={errors.author_phone ? 'error' : ''}
                        help={errors.author_phone}
                    >
                        <Input
                            size="large"
                            inputMode="numeric"
                            ...
                            onChange={(e) =>
                                setData('author_phone', sanitizePhoneInput(e.target.value))
                            }
            ...
            <Form.Item
                ...
                help={
                    errors.co_authors || 'Add up to 8 co-authors. Leave all fields empty if none.'
                }
            >
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                        ...
                        <Row key={`co-${index}`} gutter={8} align="middle">
                            <Col xs={24} md={8}>
                                <Input
                                    ...
                                    onChange={(e) =>
                                        updateCoAuthorRow(index, 'name', e.target.value)
                                    }
                                />
                            </Col>
                            <Col xs={24} md={7}>
                                <Input
                                    ...
                                    onChange={(e) =>
                                        updateCoAuthorRow(index, 'email', e.target.value)
                                    }
                                />
                            </Col>
                            <Col xs={22} md={7}>
                    ...
                    <Button
                        icon={<PlusOutlined />}
                        onClick={addCoAuthorRow}
                        disabled={coAuthorRows.length >= 8}
                        size="large"
                    >
                        Add Co-Author
                    </Button>
                ...
                <Col xs={24} md={12}>
                    ...
                    >
                        <Input
                            size="large"
                            value={data.school}
                            onChange={(event) => setData('school', event.target.value)}
            ...
            <Row gutter={16}>
                <Col xs={24} md={12}>
                    <Form.Item
                        label="Discipline"
                        validateStatus={errors.discipline ? 'error' : ''}
                        help={errors.discipline}
                    >
                        <Select
                            size="large"
                ...
                <Col xs={24} md={12}>
                    <Form.Item
                        ...
                        validateStatus={
                            errors.research_category || errors.category_type ? 'error' : ''
                        }
            ...
            >
                <Input.TextArea
                    rows={6}
                    value={data.abstract}
                    onChange={(event) => setData('abstract', event.target.value)}
                    placeholder="Summarize your research objectives, methods, and expected outcomes"
                />
            </Form.Item>
            ...
            <Form.Item
                label={
                    showCurrentFile ? 'Replace PDF (optional, max 10 MB)' : 'PDF File (max 10 MB)'
                }
